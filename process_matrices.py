...
class ProcessMatrices(torch.nn.Module):
    def __init__(self, q_s, q_c, K = None, X_E = None):
        ...
        self.V_C = torch.nn.Parameter(torch.normal(mean = 0, std = 1 / (self.n_c**2 * self.n_s**2), size = (self.n_c, self.n_s, self.n_c, self.n_s), dtype = torch.complex128))
        self.V_R = torch.nn.Parameter(torch.normal(mean = 0, std = 1 / (self.n_c**2 * self.n_s**2), size = (self.n_s, self.n_c, self.n_s, self.n_c), dtype = torch.complex128))
    ...
    def forward(self):
        X_C = opt_einsum.contract("pqmi,pqsj->misj", self.V_C.conj(), self.V_C)
        X_E = self.X_E
        X_R = opt_einsum.contract("pqil,pqjg->iljg", self.V_R.conj(), self.V_R)
        return X_C, X_E, X_R 
    ...
    def krauss_to_X_E(self, K):
        E = torch.zeros(size=self.q_c*[4] + 2*[self.n_c], dtype = torch.complex128)
